...
export const PriorityBadge = ({ priority }) => {
  ...
  return <span className={`badge ${map[priority] || 'bg-surface-hover text-ink-soft border border-surface-border'}`}>{priority}</span>;
};
...
export const EmptyState = ({ icon: Icon, title, description, action }) => (
  <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
    <div className="w-16 h-16 bg-surface-hover rounded-2xl flex items-center justify-center">
      {Icon && <Icon className="w-8 h-8 text-ink-muted" />}
    </div>
    <div>
...
export const StatCard = ({ label, value, icon: Icon, color = 'brand', trend }) => {
  const colors = {
    ...
    red: 'text-escalation-600 bg-escalation-100',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative card w-full ${sizes[size]} shadow-2xl animate-slide-up`}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-surface-border">
          ...
          <button onClick={onClose} className="text-ink-muted hover:text-ink transition-colors text-xl leading-none">&times;</button>
        </div>
